...
export default async function handler(req, res) {
  ...
  try {
    switch (event.type) {
      ...
      case "invoice.paid":
        if (session.customer) {
          await handleSubscriptionStatusChange(session.customer, true);
          console.log(`✅ Subscription payment succeeded for ${session.customer}`);
        }
        break;
      ...
      case "invoice.payment_failed":
        if (session.customer) {
          ...
          console.log(`⚠️ Subscription canceled or payment failed for ${session.customer}`);
        }
        break;
